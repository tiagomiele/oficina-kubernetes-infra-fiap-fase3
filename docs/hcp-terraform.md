...
Merges em `homolog` iniciam o workflow independente **Deploy homolog**, com jobs sequenciais de validação, infraestrutura, add-ons, observabilidade e resumo, sem aprovação manual. As dependências `needs` exibem o progresso da esquerda para a direita no gráfico e impedem uma fase de iniciar antes da anterior. Merges em `main` iniciam somente **Deploy production**, que preserva um único job e uma única aprovação no GitHub Environment `production`. Cada `workflow_dispatch` repete o deploy completo somente na branch correspondente. Destroy é manual via Terraform CLI; Auto apply do HCP continua desativado.
